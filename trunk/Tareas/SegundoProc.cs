	...
	public class SegundoProc{
		...
		public void ArmarBase(){
			...
			db.EjecutrarSecuencia(
				...
				@"INSERT INTO especificaciones (producto,especificacion,nombreespecificacion,tamannonormal) SELECT 'P' & producto,observacion,'P' & producto & ' obs ' & observacion,null FROM NovEspInf2008 n IN '"+param.NombreBaseImportacion+"' WHERE observacion>1 AND NOT EXISTS (SELECT 1 FROM especificaciones x WHERE x.producto='P' & n.producto AND x.especificacion=n.observacion) GROUP BY producto,observacion;"+
